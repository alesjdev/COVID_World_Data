...
def data_management(data):
    user_country = input("What country do you want to get information about? Type a country or press enter to exit: ")
    while user_country != "":
        if user_country in data:
            ...
            print("Sorry, can't find information about the country " + user_country + ".")

        user_country = input("Would you like to check another country? Type a country or leave blank to exit: ")
    print("Goodbye!")
...
def print_values(data, user_country, user_range, position_in_list, total_pop):
    ...
    print(" (" + percentage_of_total + "% of the country's total population)")
...
def calculate_increase_decrease(start_value, end_value):
    subtract = 100
    # If initial result is zero, we avoid DivisionByZero by changing these two values
    if start_value == 0:
        start_value = 1
        subtract = 0
    return round(end_value * 100 / start_value - subtract, 2)


def calculate_percentage(total_pop, average):
    return str(round(average * 100 / total_pop, 2))
...
# Convert date to John Hopkins' GitHub format in the csv files
...
def compute_information(final_data, date_range):
    ...
    for day in date_range:
        ...
        for row in reader:
            ...
            if row[3] not in final_data:
                inside_dictionary = {day: [confirmed, deaths, recovered, active]}
                final_data[row[3]] = inside_dictionary
            else:
                # If we don't have any values already written for that country in that given day
                if day not in final_data[row[3]]:
                    final_data[row[3]][day] = [confirmed, deaths, recovered, active]
                # If we have already written values for than country in that given day, we have multiple inputs for same country, so we add them up
                    ...
                    final_data[row[3]][day][0] += confirmed
                    final_data[row[3]][day][1] += deaths
                    final_data[row[3]][day][2] += recovered
                    final_data[row[3]][day][3] += active
    print("Done.")
    return final_data
...
def collect_countries_pop(data):
    ...
    for row in reader:
        ...
        if row[0] in data:
            ...
            for specific_date in data[row[0]]:
                try:
                    data[row[0]][specific_date].append(int(row[2]))
                # If there is any error in the values (or empty) set value to 0
                except ValueError:
                    data[row[0]][specific_date].append(0)
